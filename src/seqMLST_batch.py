...
def csvWrite(species, mlst_name, wkdir, mlst_gene_dict, sample, ST, pattern):
    outfile = os.path.join(wkdir, species + '__' + mlst_name + '__MLST.csv')
...
def main(args):
    ...
    for sample in sampleList:
        species = sample_species_Dict[sample].lower()
        for mlst_name in species_mlst_dict[species]:
            ...
            for blastdb_file in blastdb_file_list:
                ...
                query_fasta_file = recursive_fileList(wkdir, sample + '*.' + contigExt)[0]
                csvblastResults = query_fasta_file + '.blastResults.csv'
                runBlast(blastDirectory, blastdb_file, query_fasta_file, '8', '1e-1', '6', csvblastResults)
            ...
            time.sleep(3)
            csvWrite(species.replace(' ','_'), mlst_name, wkdir, mlst_gene_dict, sample, ST, pattern)
...
def run():
    ...
    parser.add_argument('-ext','--contigExt', dest="contigExt", default="fasta", help="The extension of contig files (default: fsa)")
    parser.add_argument('-s','--sampleFile', dest="sampleFile", default='', help="The tab sample file with corresponding bacterial species")
    parser.add_argument('-mlst','--mlstDir', dest="mlstDir", default='db_mlst', help="A directory for mlst schemes and data (default: mlst)")
